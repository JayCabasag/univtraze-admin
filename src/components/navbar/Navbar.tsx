...
import { Popover,PopoverContent,
  PopoverTrigger } from '@/components/shared/popover/Popover';
        ...
        <PopoverContent className='bg-white p-0 mt-[20px]'>
          <ul>
            {menuList.map((menuList) => {
              return <Link to={menuList.link} className='cursor-pointer hover:bg-gray-100 h-[50px] border-b text-[16px] flex items-center px-[15px]' key={menuList.link}>{menuList.name}</Link>
            })}
          </ul>
        </PopoverContent>
      </Popover>
